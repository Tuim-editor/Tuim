...
| `~/.local/share/tuim` | `$XDG_DATA_HOME/tuim` | Application settings (`settings.json`), active log (`tuim.log`), installed plugins (`lazy/`), Treesitter parsers (`site/`), plugin overrides (`plugin_configs/`), marketplace records, and release runtime. |
| `~/.local/state/tuim` | `$XDG_STATE_HOME/tuim` | Active session files (`sessions/`) and temporary Neovim logs. |
